...
- **`discard_draft`** — nueva content-op que descarta los cambios del draft y revierte a la versión publicada (equivale a `discard_<model>_draft` del MCP nativo). Cierra el gap de D&P del plugin en modo standalone. Requiere `confirm:true`.

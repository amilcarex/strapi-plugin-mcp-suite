...
exports.default = async (policyContext, _config, { strapi }) => {
    ...
    if (!authHeader || !authHeader.toLowerCase().startsWith("bearer ")) {
        return rejectWith("Missing Authorization header", "Header 'Authorization: Bearer <token>' faltante o malformado. Pega tu API token de Strapi.");
    }
    const token = authHeader.slice("bearer ".length).trim();
    if (!token) {
        return rejectWith("Empty bearer token", "Header 'Authorization: Bearer <token>' presente pero el token está vacío.");
    }
    const apiTokenService = strapi.service("admin::api-token");
    if (!apiTokenService || typeof apiTokenService.hash !== "function") {
        return rejectWith("Internal: api-token service unavailable", "El service 'admin::api-token' no está disponible o no expone hash(). Strapi puede haber cambiado su API interna — verificá la versión instalada (mínimo soportado: 5.0.0).");
    }
    ...
    try {
        ...
        return rejectWith("Internal: api-token lookup failed", `Error consultando el token en la DB: ${err.message}. Verificá la versión de Strapi.`);
    }
    if (!apiToken) {
        return rejectWith("Invalid API token", "El API token enviado no existe en strapi_api_tokens. Probablemente fue eliminado o nunca se creó. Crea uno nuevo en Settings → API Tokens.");
    }
    if (apiToken.expiresAt && apiToken.expiresAt < Date.now()) {
        return rejectWith("API token expired", `El token "${apiToken.name}" expiró el ${new Date(apiToken.expiresAt).toISOString()}. Crea uno nuevo en Settings → API Tokens.`);
    }
    ...
    const tokenName = (_f = apiToken.name) !== null && _f !== void 0 ? _f : "";
    ...
    try {
        ...
        owner = (_g = tokenWithOwner === null || tokenWithOwner === void 0 ? void 0 : tokenWithOwner.adminUserOwner) !== null && _g !== void 0 ? _g : null;
    }
    catch (err) {
    ...
    if (owner && emailMatch) {
        ...
        const ownerEmail = ((_h = owner.email) !== null && _h !== void 0 ? _h : "").toLowerCase();
        if (emailFromName !== ownerEmail) {
            return rejectWith("Token name email mismatch", `El email en el name del token ("${emailFromName}") no coincide con el del admin user dueño ("${ownerEmail}"). Por seguridad anti-suplantación, no se atribuye y se rechaza la request. Renombra el token con el email correcto, o quítale el email del name.`);
        }
        if (owner.isActive === false || owner.blocked === true) {
            return rejectWith("API token owner is inactive or blocked", `El admin user dueño del token (${ownerEmail}) está desactivado o bloqueado. Pídele a un admin que lo reactive, o usa otro token.`);
        }
        ...
        if (owner.isActive === false || owner.blocked === true) {
            return rejectWith("API token owner is inactive or blocked", `El admin user dueño del token está desactivado o bloqueado.`);
        }
        resolvedAdminUser = owner;
    }
    // Token sin adminUserOwner (legacy o de servicio): no atribuye. Auth válida igual.
    ctx.state = (_j = ctx.state) !== null && _j !== void 0 ? _j : {};
    ctx.state.auth = {

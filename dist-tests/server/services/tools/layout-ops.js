...
function getConfigService(strapi, uid) {
...
exports.layoutOpsTools = [
    // ── 1. get_visual_layout ────────────────────────────────────────────────────
    {
        ...
        handler: async ({ strapi }, args) => {
            ...
            return {
                uid: args.uid,
                settings: (_a = config === null || config === void 0 ? void 0 : config.settings) !== null && _a !== void 0 ? _a : null,
                metadatas: (_b = config === null || config === void 0 ? void 0 : config.metadatas) !== null && _b !== void 0 ? _b : null,
                layouts: (_c = config === null || config === void 0 ? void 0 : config.layouts) !== null && _c !== void 0 ? _c : null,
    ...
    // ── 2. set_field_layout ─────────────────────────────────────────────────────
    {
        ...
        description: "Reorganiza visualmente los campos del edit view y/o list view de un content-type o component. NO requiere reinicio de Strapi. Valida que cada campo exista en el schema, que cada size sea 1-12, y que cada fila del edit no exceda 12. Si se omite edit_rows o list_fields, se preserva el valor actual.",
        inputSchema: {
            type: "object",
            properties: {
                uid: { type: "string" },
                edit_rows: {
                    type: "array",
                    description: "Array de filas. Cada fila es array de {field, size}. size 12=100%, 6=50%, 4=33%, 3=25%. Suma de cada fila ≤ 12.",
                    items: {
                        type: "array",
                        items: {
                            type: "object",
        ...
        handler: async ({ strapi }, args) => {
            ...
            const newLayouts = { ...((_a = current.layouts) !== null && _a !== void 0 ? _a : {}) };
            if (args.edit_rows !== undefined) {
                ...
                for (const [rowIdx, row] of args.edit_rows.entries()) {
                    ...
                    if (sum > MAX_ROW_SIZE) {
                        throw new Error(`edit_rows[${rowIdx}] suma ${sum} > ${MAX_ROW_SIZE}. Reduce widths o divide en otra fila.`);
                    }
                }
                newLayouts.edit = args.edit_rows.map((row) => row.map((cell) => ({ name: cell.field, size: cell.size })));
            }
    ...
    // ── 3. set_field_metadata ───────────────────────────────────────────────────
    {
        ...
        description: "Setea label, description, placeholder, visible o editable de un campo en el edit view, y/o label, searchable, sortable en la list view. Merge parcial — los campos no enviados se preservan. NO requiere reinicio. NO modifica el schema, solo la presentación.",
        inputSchema: {
            type: "object",
            properties: {
                uid: { type: "string" },
        ...
        handler: async ({ strapi }, args) => {
            ...
            const { service } = getConfigService(strapi, args.uid);
            const current = await service.findConfiguration(model);
            const newMetadatas = { ...((_a = current.metadatas) !== null && _a !== void 0 ? _a : {}) };
            const fieldMeta = { ...((_b = newMetadatas[args.field]) !== null && _b !== void 0 ? _b : {}) };
            if (args.edit) {
                fieldMeta.edit = { ...((_c = fieldMeta.edit) !== null && _c !== void 0 ? _c : {}), ...args.edit };
            }
            if (args.list) {
                fieldMeta.list = { ...((_d = fieldMeta.list) !== null && _d !== void 0 ? _d : {}), ...args.list };
            }
            ...
            return {
                ...
                uid: args.uid,
                field: args.field,
                metadata: (_e = saved.metadatas) === null || _e === void 0 ? void 0 : _e[args.field],
                restart_required: false,
            };
        },
    },
    // ── 4. set_view_settings ────────────────────────────────────────────────────
    {
        ...
        description: "Setea opciones del list view de un content-type: mainField (campo principal mostrado), defaultSortBy, defaultSortOrder (ASC|DESC), pageSize, searchable, filterable, bulkable. Merge parcial. NO requiere reinicio. No aplicable a components.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        handler: async ({ strapi }, args) => {
            ...
            const { service } = getConfigService(strapi, args.uid);
            const current = await service.findConfiguration(model);
            const newSettings = { ...((_a = current.settings) !== null && _a !== void 0 ? _a : {}) };
            ...
            if (args.default_sort_order !== undefined)
                newSettings.defaultSortOrder = args.default_sort_order;
            if (args.page_size !== undefined)
                newSettings.pageSize = args.page_size;
            if (args.searchable !== undefined)
                newSettings.searchable = args.searchable;
            if (args.filterable !== undefined)
                newSettings.filterable = args.filterable;
            if (args.bulkable !== undefined)
                newSettings.bulkable = args.bulkable;
            const saved = await service.updateConfiguration(model, {
                ...current,

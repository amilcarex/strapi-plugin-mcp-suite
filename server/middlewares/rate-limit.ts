...
function getWindowMs(): number {
  const raw = parseInt(process.env.MCP_RATE_LIMIT_WINDOW_MS ?? "", 10);
  return Number.isFinite(raw) && raw > 0 ? raw : DEFAULT_WINDOW_MS;
}

function getPerTokenLimit(): number {
  const raw = parseInt(process.env.MCP_RATE_LIMIT_PER_MIN ?? "", 10);
  return Number.isFinite(raw) && raw > 0 ? raw : DEFAULT_PER_TOKEN_PER_MIN;
}

function getPerUserLimit(): number {
  const raw = parseInt(process.env.MCP_RATE_LIMIT_PER_USER_PER_MIN ?? "", 10);
  return Number.isFinite(raw) && raw > 0 ? raw : DEFAULT_PER_USER_PER_MIN;
}

function getPerIpLimit(): number {
  const raw = parseInt(process.env.MCP_RATE_LIMIT_PER_IP_PER_MIN ?? "", 10);
...
const ownerCache: Map<string, { ownerId: number | null; cachedAt: number }> = new Map();
...
function startCleanupIfNeeded() {
  ...
  cleanupTimer = setInterval(() => {
    ...
    for (const [key, entry] of ownerCache.entries()) {
      if (entry.cachedAt < ownerCutoff) ownerCache.delete(key);
    }
  }, CLEANUP_INTERVAL_MS);
  cleanupTimer.unref?.();
}
 ...
 */
export function checkRateLimit(key: string, customLimit?: number): LimitCheckResult {
  startCleanupIfNeeded();
  const now = Date.now();
  const windowMs = getWindowMs();
  const limit = customLimit ?? getPerTokenLimit();
 ...
 * o el lookup falla).
 */
async function getOwnerIdForToken(strapi: any, accessKeyHash: string): Promise<number | null> {
...
export default (_config: unknown, { strapi }: { strapi: Core.Strapi }) => {
  return async (ctx: any, next: () => Promise<void>) => {
    ...
    const ip = normalizeIp(ctx.request?.ip ?? ctx.ip ?? ctx.req?.socket?.remoteAddress);
...
function reject429(ctx: any, strapi: Core.Strapi, layer: "per-token" | "per-user" | "per-ip", check: LimitCheckResult) {
  ...
  strapi.log.warn(
    `[strapi-mcp] Rate limit ${layer} hit (${check.limit}/${getWindowMs() / 1000}s exceeded, retry in ${check.retryAfterSec}s)`
  );
}

function layerMessage(layer: string, limit: number, retryAfterSec: number): string {

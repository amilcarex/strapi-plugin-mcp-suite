...
export const registryTools: ToolDefinition[] = [
  {
    ...
    description:
      "Devuelve el inventario de tools del plugin: built-in (con su categoría), custom registradas por este proyecto via registerTool, y los resultados del último runSelfTests para cada tool custom que haya declarado testCases. Útil para diagnóstico — qué tools existen y cuáles pasaron sus self-tests.",
    inputSchema: {
      type: "object",
      properties: {
        include_custom_only: {
    ...
    handler: async ({ strapi }, args: any) => {
      ...
      const builtinNames: string[] = registry.getBuiltinNames?.() ?? [];

      const custom = customTools.map((t: any) => ({
        name: t.name,
        description: t.description,
        tags: Array.isArray(t.tags) ? t.tags : [],
        has_output_schema: !!t.outputSchema,
        test_cases_declared: Array.isArray(t.testCases) ? t.testCases.length : 0,
        last_self_test: summaries[t.name] ?? null,
        origin: "custom",
      }));
      ...
      const builtin = builtinNames.map((name) => {
        let category: string;
        if (name === "__health") category = "health";
        else if (name.startsWith("__")) category = "registry";
        else if (["get_visual_layout", "set_field_layout", "set_field_metadata", "set_view_settings"].includes(name))
          category = "layout-ops";
        else if (
          ["list_existing_schemas", "read_schema", "validate_schema_proposal", "create_component",
           "create_content_type", "add_field_to_schema", "delete_field_from_schema"].includes(name)
        ) category = "schema-authoring";
        else if (
          ["list_media", "get_media", "upload_media_from_url", "update_media_metadata",
      ...
      return {
        ...
        graphql_enabled: process.env.GRAPHQL_ENABLED === "true",
        gating_note:
          "Las tools de schema-authoring/upload/graphql solo aparecen en tools/list cuando su flag está en true. Acá aparecen siempre en builtin[] como referencia, junto con el estado del flag.",
      };
    },
  },
];
